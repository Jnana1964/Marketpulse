    ...
    <div className="min-h-screen overflow-x-hidden bg-[var(--color-bg)] text-white">
      <header className="mx-auto flex max-w-7xl items-center justify-between px-6 py-6 lg:px-10">
        <div className="flex items-center gap-2.5">
          <Activity size={26} strokeWidth={2.5} className="text-[var(--color-positive)]" />
        ...
        <div className="flex items-center gap-3">
          ...
          <Link to="/signup" className="flex items-center gap-1.5 rounded-full bg-[var(--color-positive)] px-5 py-2.5 text-base font-semibold text-black transition hover:opacity-90">
            Get Started →
          </Link>
        </div>
      </header>

      <main id="home" className="relative mx-auto max-w-7xl px-6 pb-20 pt-12 text-center lg:px-10">
        <div className="pointer-events-none absolute inset-x-0 top-0 -z-10 h-[420px] bg-brand-glow" />

        <span className="inline-flex items-center gap-2 rounded-full border border-[var(--color-border-accent)] bg-[var(--color-positive-soft)] px-5 py-2 text-sm font-semibold text-[var(--color-positive)]">
          <span className="h-2 w-2 rounded-full bg-[var(--color-positive)]" /> Smarter insights. Clearer decisions.
        </span>
        ...
        <p className="mt-3 text-2xl font-semibold text-white sm:text-3xl">See what changed, not just what moved.</p>
        ...
        <div className="mt-9 flex items-center justify-center gap-4">
          <Link to="/signup" className="flex items-center gap-2 rounded-full bg-[var(--color-positive)] px-8 py-4 text-base font-bold text-black transition hover:opacity-90">
            Get Started →
          </Link>
      ...
      <section id="features" className="mx-auto max-w-7xl px-6 py-4 lg:px-10">
        <div className="flex items-center justify-between rounded-2xl border border-[var(--color-border)] bg-[var(--color-card)] p-7">
          <div className="flex items-center gap-4">
            <span className="flex h-14 w-14 flex-none items-center justify-center rounded-full bg-[var(--color-positive-soft)] text-[var(--color-positive)]">
              <Activity size={26} />
            </span>
            <div>
        ...
        <div className="mt-6 grid grid-cols-2 gap-5 lg:grid-cols-4 lg:gap-6">
          {FEATURES.map(({ icon: Icon, title, description }) => (
            <div key={title} className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-card)] p-7">
              <span className="flex h-14 w-14 items-center justify-center rounded-full bg-[var(--color-positive-soft)] text-[var(--color-positive)]">
                <Icon size={26} />
              </span>
      ...
      <section id="how-it-works" className="mx-auto max-w-7xl px-6 py-16 lg:px-10">
        <p className="text-center text-sm font-semibold uppercase tracking-widest text-[var(--color-positive)]">How It Works</p>
          ...
          {STEPS.map(({ icon: Icon, title, description }, i) => (
            <div key={title} className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-card)] p-7">
              <div className="flex items-center justify-between">
                ...
                <Icon size={26} className="text-[var(--color-positive)]" />
              </div>
              <h3 className="mt-5 text-lg font-semibold text-white">{title}</h3>
              <p className="mt-2 text-base text-[var(--color-text-secondary)]">{description}</p>
            </div>
      ...
      <section id="about" className="mx-auto max-w-5xl px-6 pb-20 lg:px-10">
        <div className="relative overflow-hidden rounded-2xl border border-[var(--color-border-accent)] bg-[var(--color-card-alt)] bg-brand-glow px-8 py-16 text-center">
          ...
          <h2 className="mt-4 text-4xl font-extrabold text-white sm:text-5xl">
            Know where to look. <span className="text-[var(--color-positive)]">Know why it matters.</span>
          </h2>
          <Link
            to="/signup"
            className="mt-8 inline-flex items-center gap-2 rounded-full bg-[var(--color-positive)] px-8 py-4 text-base font-bold text-black transition hover:opacity-90"
          >
            Get Started →
          </Link>
        </div>
      ...
      <footer className="border-t border-[var(--color-border)] px-6 py-10 text-center text-sm text-[var(--color-text-muted)]">
        <div className="flex items-center justify-center gap-2.5 pb-3 text-lg font-semibold text-white">
          <TrendingUp size={20} className="text-[var(--color-positive)]" /> MarketPulse
        </div>
